...
def contar_imagenes(codigo):
    """Cuenta cuántas imágenes existen para un código, considerando sufijos _1, _2..."""
    contador = 0
...
def obtener_ruta_imagen(codigo, indice):
    """Devuelve la ruta correcta según el índice de la imagen"""
    if indice == 1:
...
# Registrar funciones para que funcionen en el HTML
...
def index():
    productos = get_data()
    # Extraer categorías, públicos y versiones únicas
    categorias = sorted(list(set(p['CATEGORIA'] for p in productos if p.get('CATEGORIA'))))
    publicos = sorted(list(set(p['PUBLICO'] for p in productos if p.get('PUBLICO'))))
    versiones = sorted(list(set(p['VERSION'] for p in productos if p.get('VERSION'))))
